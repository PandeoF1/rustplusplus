...
class WebServer {
    ...
    setupRoutes() {
        ...
        this.app.post('/api/server/:guildId/notification-settings', async (req, res) => {
            const { guildId } = req.params;
        ...
        this.app.post('/api/switch/:guildId/:entityId', async (req, res) => {
            ...
            try {
                ...
                if (targetState === undefined) {
                    ...
                    const instance = this.client.getInstance(guildId);
                    const current = instance.serverList[rustplus.serverId]?.switches[entityId]?.active;
                    targetState = !current;
                }

                const response = await rustplus.turnSmartSwitchAsync(entityId, targetState);
        ...
        this.app.post('/api/switch/:guildId/:entityId/edit', async (req, res) => {
            try {
                ...
                if (command && command !== server.switches[entityId].command &&
                    !Keywords.getListOfUsedKeywords(this.client, guildId, rustplus.serverId).includes(command)) {
                ...
                if (autoDayNightOnOff !== undefined && autoDayNightOnOff !== null) {
                    server.switches[entityId].autoDayNightOnOff = parseInt(autoDayNightOnOff);
                }
            ...
            try {
                ...
                for (const [groupId, content] of Object.entries(server.switchGroups)) {
                    if (content.switches.includes(entityId.toString())) {
                        server.switchGroups[groupId].switches = content.switches.filter(e => e !== entityId.toString());
                        this.client.setInstance(guildId, instance);
        ...
        this.app.get('/api/battlemetrics/players', async (req, res) => {
            ...
            try {
                ...
                const searchUrl = `https://api.battlemetrics.com/servers?filter[search]=${encodeURIComponent(q)}&page[size]=1&include=player`;
        ...
        this.app.get('/api/battlemetrics/players/:guildId', async (req, res) => {
            ...
            try {
                ...
                if (!client.rustplusInstances[guildId]) {
                    return res.status(404).json({ error: 'Rust+ instance not found for guild' });
                }
                ...
                if (!server || !server.battlemetricsId) {
                    return res.status(404).json({ error: 'BattleMetrics ID not configured for this server' });
                }
                ...
                res.status(500).json({ error: 'Internal failure accessing BattleMetrics data' });
            }
        });
        ...
        this.app.post('/api/tracker/:guildId/create', async (req, res) => {
            try {
                ...
                if (newChannel) {
                    ...
                    res.json({ success: true, trackerId, tracker: instance.trackers[trackerId] });
                } else {
        ...
        this.app.post('/api/tracker/:guildId/:trackerId/edit', async (req, res) => {
            try {
                ...
                if (battlemetricsId && battlemetricsId !== tracker.battlemetricsId) {
                    if (this.client.battlemetricsInstances.hasOwnProperty(battlemetricsId)) {
                        const bmInstance = this.client.battlemetricsInstances[battlemetricsId];
        ...
        /* Delete a tracker */
        this.app.post('/api/tracker/:guildId/:trackerId/delete', async (req, res) => {
            try {
                const { guildId, trackerId } = req.params;
                const instance = this.client.getInstance(guildId);
                const tracker = instance.trackers[trackerId];
    ...
    getServerData(guildId, useCache = true) {
        ...
        const data = {
            ...
            trackers: this.enrichTrackersWithPlayerData(guildId, instance.trackers || {})
        };
        ...
        for (const [trackerId, tracker] of Object.entries(enriched)) {
            ...
            tracker.players = tracker.players.map(player => {
                const bmPlayer = bmInstance.players[player.playerId] ||
                    Object.values(bmInstance.players).find(p => p.steamId === player.steamId);
    ...
    broadcastGuildsUpdate() {
        this.client.log(this.client.intlGet(null, 'infoCap'), 'WebUI: broadcasting guilds update to all clients');
    ...
    broadcastNotification(guildId, type, message) {
        this.client.log(this.client.intlGet(null, 'infoCap'), `WebUI: broadcasting notification to guild ${guildId}: ${type}`);
        this.io.to(`guild-${guildId}`).emit('notification', { type, message });
    }

    broadcastChatMessage(guildId, message) {
        this.client.log(this.client.intlGet(null, 'infoCap'), `WebUI: broadcasting chat message to guild ${guildId} from ${message.player_name}`);
